...
export class ProjectService {
  ...
  getAllProjectsFromUser(idUser: String) {
    return new Promise((resolve, reject) => {
      this.http.post('http://localhost:5000/api/get-projects/', { _id: idUser }).subscribe(
        (response) => {
          resolve(response);
        },
        (error) => {
  ...
  addUserToProject(idProject: String, idUser: String) {
    return new Promise((resolve, reject) => {
      this.http.post('http://localhost:5000/api/project/' + idProject + '/add-user', { _id: idUser }).subscribe(
        (response) => {
          resolve(response);
        },
        (error) => {
